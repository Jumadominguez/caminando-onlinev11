...
productSchema.index({ ean: 1 }); // Índice para búsquedas por EAN
productSchema.index({ sku: 1 }); // Índice para búsquedas por SKU
...
let Product;
if (processed) {
  Product = processed.model('Product', productSchema);
} else {
  // Fallback para desarrollo - usar conexión por defecto
  console.warn('⚠️  Processed connection not available, using default connection for Product model');
