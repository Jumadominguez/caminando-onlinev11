...
let PriceHistory;
if (processed) {
  PriceHistory = processed.model('PriceHistory', priceHistorySchema);
} else {
  // Fallback para desarrollo - usar conexión por defecto
  console.warn('⚠️  Processed connection not available, using default connection for PriceHistory model');
