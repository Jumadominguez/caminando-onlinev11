...
// Función para obtener el modelo Subcategory (evita registro duplicado)
function getSubcategoryModel() {
  if (carrefour) {
    // Verificar si el modelo ya existe en la conexión carrefour
    try {
      return carrefour.model('Subcategory');
    } catch (error) {
      // Si no existe, crearlo
      return carrefour.model('Subcategory', subcategorySchema);
    }
  } else {
    ...
    try {
      return mongoose.model('Subcategory');
    } catch (error) {
