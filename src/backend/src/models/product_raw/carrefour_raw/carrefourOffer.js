...
// Función para obtener el modelo Offer (evita registro duplicado)
function getOfferModel() {
  if (carrefour) {
    // Verificar si el modelo ya existe en la conexión carrefour
    try {
      return carrefour.model('Offer');
    } catch (error) {
      // Si no existe, crearlo
      return carrefour.model('Offer', offerSchema);
    }
  } else {
    // Fallback para desarrollo - usar conexión por defecto
    console.warn('⚠️  Carrefour connection not available, using default connection for Offer model');
    try {
